...
function getChallengeAnswers(challenge: Record<string, unknown>): string[] {
  ...
  if (
    ...
    const choice: unknown = (choices as unknown[])[correctIndex];
    if (isRecord(choice)) {
      const choiceText =
...
function extractPracticeSentence(
...
async function samplePracticeSentences(
  client: DuolingoClient,
  languageAbbr: string,
  fromLanguage: string,
  sessions: number,
  ...
  const responses = await Promise.all(
    Array.from({ length: sessions }, () =>
      client.getGlobalPracticeSession(languageAbbr, fromLanguage),
    ),
  );
